...
$.extend($.fn.validatebox.defaults.rules, {
	...
	mobile : {
		validator : function(value, param) {
			...
			/^(1)\d{10}$/.test(value)?addValidateHint($(this)):removeValidateHint($(this));
	...
	tel : {
		validator : function(value, param) {
			/^(([0]\d{2,3}-\d{7,8})|([0]\d{2,3}\d{7,8}))$/.test(value)?addValidateHint($(this)):removeValidateHint($(this));
	...
	idcard : {
		validator : function(value, param) {
			idCard(value)?addValidateHint($(this)):removeValidateHint($(this));
			return idCard(value);
	...
	cardNum:{
		validator : function(value, param) {
			/^\d{16,19}$/.test(value)?addValidateHint($(this)):removeValidateHint($(this));
	...
	accountNum:{
		validator:function(value,param){
			 isAccountNum(value)?addValidateHint($(this)):removeValidateHint($(this));
			 return isAccountNum(value);
		},
		message:'请输入正确的账号！'
	},
	regNoOrRegUSCI:{
		validator:function(value,param){
			 isRegNoOrRegUSCI(value)?addValidateHint($(this)):removeValidateHint($(this));
			 return isRegNoOrRegUSCI(value);
		},
		message:'请输入正确的营业执照号'
	},
    flEmpty: {
...
var isAccountNum=function(value){
	var reg = new RegExp("[\\u4E00-\\u9FFF]+","g");
	var regRegNo=new RegExp("[0-9]{15}$");
	var regUSCI=new RegExp("(([159][1239])|([234678ABCDEFGY]1))[0-9]{6}[0-9A-HJ-NP-RTUW-Y]{10}$");
	if(!reg.test(value)){
		 //不是中文名（非商户名）
		 if(value.length<=15 && !regRegNo.test(value)){
			 //营业执照
			return false;
		 }
		 if(value.length>15 && !regUSCI.test(value)){
			 //信用代码
		 	return false;
		 }
	}
...
var isRegNoOrRegUSCI = function(value){
	var regRegNo=new RegExp("[0-9]{15}$");
	var regUSCI=new RegExp("(([159][1239])|([234678ABCDEFGY]1))[0-9]{6}[0-9A-HJ-NP-RTUW-Y]{10}$");
	if(value.length<=15 && !regRegNo.test(value)){
		 //营业执照
...
var addValidateHint = function(obj){
	if($(obj).parent('.textbox').parent().find('.icon-validate')[0]) return;
	$(obj).parent('.textbox').parent().append('<i class="icon-validate icon-hint-ok"></i>');
};
var removeValidateHint = function(obj){
	if($(obj).parent('.textbox').parent().find('i')){
		$(obj).parent('.textbox').parent().find('i').remove();
	}
};